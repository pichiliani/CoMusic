	...
	private void paintScoreNote(ViewLayout layout,TGPainter painter, int fromX, int fromY, int spacing) {
		...
		if((layout.getStyle() & ViewLayout.DISPLAY_SCORE) != 0 ){
			...
			if(getEffect().isHarmonic()){
				...
				/********************************************/
				// Aqui faz a pintura da bolinha da nota!
				/********************************************/
				if( layout.isBufferEnabled() )
				{
					// TODO: OK, por algum motivo não chega aqui quando acanço para a direita automaticamente!
					// Passando o usuário que criou esta nota!
					painter.drawImage(layout.getResources().getScoreNote(getBeat().getDuration().getValue(),playing, this),x,y1);
				}
				else
				{
					boolean full = (getBeat().getDuration().getValue() >= TGDuration.QUARTER);
