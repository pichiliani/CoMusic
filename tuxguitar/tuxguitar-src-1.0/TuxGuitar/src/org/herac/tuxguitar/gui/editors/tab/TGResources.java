...
public class TGResources {
	...
	public Color getScoreNoteColor() {
		// Aqui é onde se obterm a cor da nota
		// A idéia é pegar a cor de nota do usuário que enviou a última mensagem
		 ...
		 } catch (Exception e) 
		 {
	      	...
	      	System.out.println("getScoreNoteColor() de TGResources.java");
			 e.printStackTrace();
	      } */    

		if( TuxGuitar.instance().clienteEnvia.getConected() )
		{
			if(TuxGuitar.instance().clienteEnvia.getCorAtual() == null)
				return this.scoreNoteColor;
			else
				return TuxGuitar.instance().clienteEnvia.getCorAtual();
		}
		else
		{
			return this.scoreNoteColor;
	...
	public Image getScoreNote(int value,boolean playing, TGNote note) {
		int index = 0;
		index += ((playing)?1:0);
		index += ((value >= TGDuration.QUARTER)?2:0);
		
		// Aqui é preciso realizar um controle da nota
		// Verifico quem é o proprietário da nota e determino sua cor
		// O tuxguitar desenha todas as todas do mesmo compasso quando uma nova nota é criada!
		
		if( TuxGuitar.instance().clienteEnvia.getConected() )
		{
			TuxGuitar.instance().clienteEnvia.setCorAtual( TuxGuitar.instance().clienteEnvia.retornaCor(  note.getUser() ) );
	...
	private void initImages(){
		...
		// Aqui são colocadas as cores das bolinhas das notas!
